...
function AdminRegister({ handleCloseModal }: IProps) {
    ...
    useEffect(() => {
        ...
        setValid(dateGenerate !== "undefined-1-undefined" && getGender !== "");
    }, [dateValue, genderValue]);
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            ...
            password: Yup.string().required('Password can not be empty').min(6, 'Min is 6 characters').max(12, 'Max is 12 characters').matches(regex.password, 'Password must contain at least 1 digit, 1 special character, 1 alphabetic character!'),
    ...
    return (
        <div className='admin-register'>
            <form className='admin-register-form' onSubmit={formik.handleSubmit}>
                <h1 className='header'>Form Đăng Ký Admin</h1>
                <Grid container spacing={2} className='mui-grid-admin'>
                    <Grid item lg={6} className='mui-item-grid-admin'>
                        <FormControl variant='standard' className='mui-form-control-admin' margin='dense' error={!!formik.touched?.user && !!formik.errors?.user}>
                            ...
                            {formik.touched?.user && formik.errors?.user ? <FormHelperText id="my-helper-text">{formik.errors.user}</FormHelperText> : null}
                        </FormControl>
                    </Grid>
                    <Grid item lg={6} className='mui-item-grid-admin'>
                        <FormControl variant='standard' className='mui-form-control-admin' margin='dense' error={!!formik.touched?.password && !!formik.errors?.password}>
                            ...
                            {formik.touched?.password && formik.errors?.password ? <FormHelperText id="my-helper-text">{formik.errors.password}</FormHelperText> : null}
                        </FormControl>
                    </Grid>
                    <Grid item lg={6} className='mui-item-grid-admin'>
                        <FormControl variant='standard' className='mui-form-control-admin' margin='dense' error={!!formik.touched?.checkPassword && !!formik.errors?.checkPassword}>
                            ...
                            {formik.touched?.checkPassword && formik.errors?.checkPassword ? <FormHelperText id="my-helper-text">{formik.errors.checkPassword}</FormHelperText> : null}
                        </FormControl>
                    </Grid>
                    <Grid item lg={6} className='mui-item-grid-admin'>
                        <FormControl variant='standard' className='mui-form-control-admin' margin='dense' error={!!formik.touched?.name && !!formik.errors?.name}>
                    ...
                    <Grid item lg={6} className='mui-item-grid-admin'>
                        <FormControl variant='standard' className='mui-form-control-admin' margin='dense' error={!!formik.touched?.email && !!formik.errors?.email}>
                    ...
                    <Grid item lg={6} className='mui-item-grid-admin'>
                        <FormControl variant='standard' className='mui-form-control-admin' margin='dense' error={!!formik.touched?.phone && !!formik.errors?.phone}>
                    ...
                    <Grid item lg={6} className='mui-item-grid-admin'>
                        ...
                        {!isValid ? <FormHelperText id="my-helper-text" style={{ color: "red", fontSize: "1.5rem" }}>Bạn chưa điền ngày sinh hoặc giới tính</FormHelperText> : null}
                    </Grid>
                </Grid>

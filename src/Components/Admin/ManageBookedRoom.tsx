...
function ManageBookedRoom() {
  ...
  const bookingListData = useSelector((state: RootState) => state.sliceBookingAdmin?.listBooking);
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'ngayDi', headerName: 'Ngày đi', width: 180, align: 'center', headerAlign: 'center' },
    { field: 'soLuongKhach', headerName: 'Số lượng khách', type: 'number', width: 120, align: 'center' },
    { field: 'maNguoiDung', headerName: 'Mã khách hàng', width: 120, align: 'center' },
    {
      field: "action",
      type: 'actions',
      headerName: "Action",
  ...
  const handleSearchRoom = () => {
    if (valueInput !== "") {
      const arraySearch: IBookRoom[] = newRows.filter((item) => item?.maPhong === Number(valueInput));
      setArr(arraySearch);
    } else {
      setArr(newRows);
    }

...
function ExploreLocation() {
  const result: ILocationItem[] = useGetInspectOfSearchPage() || []; // Ensure result is an array
          ...
          <Grid container spacing={2} className="mui-grid">
            {result.map((itemReturn: ILocationItem) => (
              <Grid
                ...
                lg={3}
                md={4}
                className="mui-grid-item item-location"
                key={itemReturn.id}
              >
                <Item className="mui-item item-location-detail">
                  <NavLink to={`${itemReturn.id}`}>
                    <div className="explore-location-item">
                      <img src={itemReturn?.hinhAnh ?? 'default-image.jpg'} alt="..." />
                      <div className="explore-location-item-text">
                        <p>{itemReturn?.tenViTri ?? 'Unknown Location'}</p>
                        <span>{`${itemReturn?.tinhThanh ?? 'Unknown City'}, ${itemReturn?.quocGia ?? 'Unknown Country'}`}</span>
                      </div>
                    </div>
                  </NavLink>
                </Item>

...
import { filterLocations } from './modalLocationLogic';
...
export default function BasicModal({ children, value, setValue }: TProps) {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const findResultList = filterLocations(listData, e.target.value);
            ...
            >
                <Box sx={style} className='mui-box-location'>
                    <Typography id="modal-modal-title" variant="h6" component="h2">
                        Địa điểm khả dụng: <input type="text" value={value} onChange={handleChange} />
                    </Typography>

...
import { IBookRoom, IProfile } from '../../constant/constant';
            ...
            <div className="profile-page row mb-5">
                    ...
                    <div className="profile-fixed">
                        <div className="profile-avatar">
                            ...
                            <div className="update-avatar">
                                ...
                                <Button variant="contained" onClick={handleUpdateAva}>
                                    Cập nhật ảnh
                                </Button>
                            </div>
                        </div>
                        <div className="profile-section-description">
                            ...
                            <p className="mt-3">
                                <i className="fa fa-check" aria-hidden="true"></i> Địa chỉ Email
                            </p>
                        </div>
                    </div>
                </div>
                <div className="profile-bookRoom-section col-lg-9 pl-lg-5 mt-5 mt-lg-0">
                    ...
                    {hide ? (
                        ...
                        <div>
                            {data.length >= 0 &&
                                data.map((booked: IBookRoom, index: number) => {
                                    return <BookRoom index={index} key={booked?.id} bookRoom={booked} />;
                                })}

                            <div className="d-flex justify-content-center mt-5">
                                {bookRoomData && bookRoomData.length / PAGE_SIZE > 1 && (
                                    <Pagination total={bookRoomData.length} current={page} pageSize={PAGE_SIZE} onChange={(page) => setPage(page)} />
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    {bookRoomData?.length === 0 ? (
                        ...
                        >
                            {hide ? `Số phòng bạn đã đặt là ${bookRoomData?.length}` : 'Ẩn hiện thị phòng'}
                        </button>
                    )}
                </div>
            </div>
